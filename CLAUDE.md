...
| POST | `/api/restore/full` | Body: raw `application/gzip` archive from `/api/backup/full`. Validates it (migrate, integrity-check, verify every live table is readable) before installing; never touches the live DB file if validation fails. Requires a server restart to take effect — see `server/lib/fullRestore.js`. |

...
describe("dialog accessibility", () => {
  ...
  it("exposes a live status region and labelled board/column/card groups", async () => {
    ...
    expect(statusRegions.some((el) => /Board ready/.test(el.textContent))).toBe(
      true,
    );
    expect(
      screen.getByRole("group", { name: "Repository board" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: /Today column/ }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("group", { name: /alpha — review/ }),
    ).toBeInTheDocument();
  });
  ...
  it("rate-limit detail is accessible via the status dialog", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Open dashboard status" }),
    );
    expect(
      await screen.findByRole("dialog", { name: "Dashboard status" }),
    ).toBeInTheDocument();

...
describe("App cache hydration", () => {
  ...
  it("shows cached data immediately and updates when API payload arrives", async () => {
    ...
    expect(
      screen.getByText("Showing cached board while refreshing from GitHub."),
    ).toBeInTheDocument();
    ...
    expect(
      screen.queryByText("Showing cached board while refreshing from GitHub."),
    ).not.toBeInTheDocument();

    const cached = JSON.parse(
      window.localStorage.getItem("repo-triage-board-cache-v1"),
    );

...
describe("App filter persistence", () => {
  ...
  it("persists filter changes to localStorage and restores on re-render", () => {
    ...
    expect(
      JSON.parse(window.localStorage.getItem("repo-triage-filters")).showOwn,
    ).toBe(false);
  ...
  it("double-click solos a pill: turns the other two off and this one on", () => {
    ...
    const forksPill = screen
      .getByRole("checkbox", { name: "forks" })
      .closest("label");
    ...
    expect(
      JSON.parse(window.localStorage.getItem("repo-triage-filters")),
    ).toEqual({
      showOwn: false,
      showForks: true,
      showArchived: false,

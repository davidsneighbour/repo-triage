...
describe("mobile board", () => {
  ...
  it("switches the visible column via the day picker", async () => {
    ...
    expect(
      screen.queryByRole("link", { name: "due-now" }),
    ).not.toBeInTheDocument();
  });

  it("does not render the desktop multi-column scroll layout", async () => {
    api.list.mockResolvedValue(
      payload([card({ id: 1, name: "due-now", needsCheckToday: true })]),
    );
    ...
    expect(
      within(board).getAllByRole("group", { name: /column,/ }),
    ).toHaveLength(1);
  });
});
...
describe("mobile toolbar (collapsed into an action sheet)", () => {
  ...
  it("hides the secondary controls until the overflow sheet is opened", async () => {
    api.list.mockResolvedValue(
      payload([card({ id: 1, name: "r", needsCheckToday: true })]),
    );
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "More filters and options" }),
    );
    ...
    expect(
      within(sheet).getByLabelText("Sort cards within columns"),
    ).toBeInTheDocument();
    expect(
      within(sheet).getByRole("button", { name: /reports/ }),
    ).toBeInTheDocument();
    expect(
      within(sheet).getByRole("button", { name: /notices/ }),
    ).toBeInTheDocument();
    expect(
      within(sheet).getByRole("button", { name: /show ignored/ }),
    ).toBeInTheDocument();
...
describe("desktop board (above the breakpoint)", () => {
  ...
  it("renders all day columns and no day picker", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: /Choose day/ }),
    ).not.toBeInTheDocument();
    const board = screen.getByRole("group", { name: "Repository board" });
    // 7-day review cycle → 7 columns visible at once on desktop.
    expect(
      within(board).getAllByRole("group", { name: /column,/ }).length,
    ).toBe(7);
  });
});

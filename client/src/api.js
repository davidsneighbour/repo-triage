...
export const api = {
  ...
  clearSchedule: (id) =>
    fetch(`/api/repos/${id}/clear`, { method: "POST" }).then(json),
  ...
  allNotices: (sort = "date", dir = "desc") =>
    fetch(
      `/api/notices?sort=${encodeURIComponent(sort)}&dir=${encodeURIComponent(dir)}`,
    ).then(json),
  deleteNotice: (noticeId) =>
    fetch(`/api/notices/${noticeId}`, { method: "DELETE" }).then(json),
  addTag: (id, tag) =>
  ...
  removeTag: (id, tag) =>
    fetch(`/api/repos/${id}/tags/${encodeURIComponent(tag)}`, {
      method: "DELETE",
    }).then(json),
  ...
  deleteTag: (tag, resetCheck = false) =>
    fetch(
      `/api/tags/${encodeURIComponent(tag)}${resetCheck ? "?resetCheck=true" : ""}`,
      { method: "DELETE" },
    ).then(json),
  addFlag: (id, flag) =>
  ...
  removeFlag: (id, flag) =>
    fetch(`/api/repos/${id}/flags/${encodeURIComponent(flag)}`, {
      method: "DELETE",
    }).then(json),
  ghOpen: (id) =>
    fetch(`/api/repos/${id}/gh/open`, { method: "POST" }).then(json),
  ...
  discardUndo: (id) =>
    fetch(`/api/undo/${id}`, { method: "DELETE" }).then(json),
  getLastExport: () => fetch("/api/reports/last-export").then(json),
  repoIssues: (id) => fetch(`/api/repos/${id}/issues`).then(json),
  allIssues: () => fetch("/api/issues").then(json),
  syncRepoIssues: (id) =>
    fetch(`/api/repos/${id}/issues/sync`, { method: "POST" }).then(json),
  ...
  getRepoConformance: (id, presetId) =>
    fetch(
      `/api/repos/${id}/settings-sets/${encodeURIComponent(presetId)}`,
    ).then(json),
};

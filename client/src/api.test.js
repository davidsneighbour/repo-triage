...
describe("api wrapper contract", () => {
  ...
  it("calls allIssues endpoint with GET", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue({ json: async () => ({ issues: [] }) });
  ...
  it("calls allActivity endpoint with GET", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue({ json: async () => ({ activity: [] }) });
  ...
  it("builds report URLs and parses json vs text per format", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "/api/reports/stale?format=md&days=90",
    );
  });
  ...
  it("POSTs to gh/open endpoint", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith("/api/repos/42/gh/open", {
      method: "POST",
    });
  });
  ...
  it("GETs gh/prs endpoint", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue({ json: async () => ({ prs: [] }) });
  ...
  it("POSTs gh/issue with title and body", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue({
      json: async () => ({ ok: true, url: "u", number: 1 }),
    });
  ...
  it("fetches prefs with GET /api/prefs", async () => {
    const fetchMock = vi
      .spyOn(globalThis, "fetch")
      .mockResolvedValue({ json: async () => ({ prefs: null }) });
  ...
  it("covers clearSchedule, restoreState, deleteTag, addFlag, removeFlag", async () => {
    ...
    expect(calls[1]).toEqual([
      "/api/repos/1/state",
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          priority_set_at: "2026-05-01T00:00:00.000Z",
    ...
    expect(calls[3][1]).toMatchObject({
      method: "POST",
      body: JSON.stringify({ flag: "pinned" }),
    });
    expect(calls[4]).toEqual([
      "/api/repos/1/flags/pinned",
      { method: "DELETE" },
    ]);
  });
  ...
  it("covers getSettingsSets and getRepoConformance", async () => {
    ...
    expect(fetchMock.mock.calls[1]).toEqual([
      "/api/repos/1/settings-sets/hygiene",
    ]);
  });
  ...
  it("covers createTag, renameTag, and deleteTag with resetCheck", async () => {
    ...
    expect(calls[2]).toEqual([
      "/api/tags/platform?resetCheck=true",
      { method: "DELETE" },
    ]);
  });
  ...
  it("covers getActivity, getUndoLog, createUndo, executeUndo, discardUndo", async () => {
    ...
    await api.createUndo("test", [
      { type: "setIgnored", repoId: 1, ignored: false },
    ]);

...
export function BulkBar({ count, actions, columns = [], onClear }) {
  ...
  return (
    ...
    >
      ...
      <button className={btn} onClick={actions.checkedNow}>
        Checked now
      </button>
      {columns.length > 0 ? (
        <label className="flex items-center gap-1">
          ...
          >
            ...
            {columns.map((c) => (
              <option key={c.key} value={c.daysAgoTarget}>
                {c.title}
              </option>
            ))}
          </select>
        ...
        <button className={btn} onClick={actions.moveToday}>
          Move to Today
        </button>
      )}
      <button className={btn} onClick={actions.clear}>
        Clear check
      </button>
      <label className="flex items-center gap-1">
        ...
        >
          ...
          {PRIORITY_FILTER_OPTIONS.map((o) => (
            <option key={o.level} value={o.level === 0 ? "null" : o.level}>
              {o.label}
            </option>
          ))}
        </select>
      </label>
      <button className={btn} onClick={actions.ignore}>
        Ignore
      </button>
      <button className={btn} onClick={actions.unignore}>
        Unignore
      </button>

...
describe("CardMenu GitHub actions — PR list", () => {
  ...
  it("shows PR button when onGhPrs is provided", () => {
    render(
      <CardMenu
        repo={baseRepo}
        anchorRef={anchor}
        {...makeHandlers({ onGhPrs: vi.fn() })}
      />,
    );
    expect(
      screen.getByRole("button", { name: /List open PRs/i }),
    ).toBeInTheDocument();
  });
  ...
  it("fetches and displays PR list on click", async () => {
    ...
    expect(
      screen.getByRole("button", { name: /Hide PRs/i }),
    ).toBeInTheDocument();
  });
  ...
  it("shows error alert when onGhPrs rejects", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /List open PRs/i }));
    await waitFor(() =>
      expect(screen.getByRole("alert")).toHaveTextContent(/gh failed/),
    );
  });
  ...
  it('shows "Browse issues" button when onViewIssues is provided', () => {
    render(
      <CardMenu
        repo={baseRepo}
        anchorRef={anchor}
        {...makeHandlers({ onViewIssues: vi.fn() })}
      />,
    );
    expect(
      screen.getByRole("button", { name: "Browse issues" }),
    ).toBeInTheDocument();
  });
  ...
  it("calls onViewIssues with the repo id and closes the menu", () => {
    ...
    render(
      <CardMenu
        repo={baseRepo}
        anchorRef={anchor}
        {...makeHandlers({ onViewIssues, onClose })}
      />,
    );
...
describe("CardMenu GitHub actions — issue creation", () => {
  ...
  it('shows "New issue…" button when onGhCreateIssue is provided', () => {
    render(
      <CardMenu
        repo={baseRepo}
        anchorRef={anchor}
        {...makeHandlers({ onGhCreateIssue: vi.fn() })}
      />,
    );
    expect(
  ...
  it('clicking "New issue…" shows the issue form', () => {
    ...
    expect(
      screen.getByRole("textbox", { name: "Issue body" }),
    ).toBeInTheDocument();
  });
  ...
  it('"Yes, create" calls onGhCreateIssue and shows success link', async () => {
    const onGhCreateIssue = vi.fn().mockResolvedValue({
      url: "https://github.com/me/alpha/issues/42",
      number: 42,
    });
    render(
      <CardMenu
    ...
    await waitFor(() =>
      expect(screen.getByText(/Issue #42 created/)).toBeInTheDocument(),
    );

...
export function CardMenu({
  ...
  useEffect(() => {
    ...
    onGetConformance(repo.id, settingsSets[0].id)
      .then((result) => {
        if (!cancelled) setConformance({ result });
      })
      .catch(() => {
        if (!cancelled) setConformance({ error: true });
      });
    return () => {
      cancelled = true;
    };
  ...
  return createPortal(
    <>
      ...
      <div
        ...
        role="dialog"
        aria-label={
          tagOnly ? `Tags for ${repo.name}` : `Settings for ${repo.name}`
        }
        tabIndex={-1}
        className={cx(
      ...
      >
        {!tagOnly && (
          <>
            <p className="px-1 pb-1 text-[10px] uppercase tracking-widest text-neutral-500">
              Review timing
            </p>
            ...
            <div className="mt-2 border-t border-neutral-800 pt-2">
              <label className="block px-1 text-[10px] uppercase tracking-widest text-neutral-500">
                Priority
              </label>
              <div
                ...
                {PRIORITY_LEVELS.map((level) => {
                  ...
                  return (
                    <button
                      ...
                      title={meta.title}
                      onClick={() =>
                        onSetPriority(repo.id, active ? null : level)
                      }
                      className={cx(
            ...
            <div className="mt-2 border-t border-neutral-800 pt-2">
              ...
              <p className="mt-1 px-1 text-[10px] text-neutral-600">
                Blank = default ({defaultInactivity}d)
              </p>
            </div>
          </>
        )}
        ...
        <div
          className={cx("mt-2 pt-2", !tagOnly && "border-t border-neutral-800")}
        >
          <label className="block px-1 text-[10px] uppercase tracking-widest text-neutral-500">
            Tags
          </label>
          {repo.tags?.length > 0 && (
            <div className="mt-1 flex flex-wrap gap-1">
              {repo.tags.map((t) => (
                <span
                  key={t}
                  className="inline-flex items-center gap-1 rounded-sm bg-neutral-800 px-1.5 py-0.5 text-[10px] text-neutral-300"
                >
                  <span
          ...
          <>
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <>
                      ...
                      >
                        {prsState === "loading"
                          ? "loading PRs…"
                          : prsState?.prs
                            ? `Hide PRs (${prsState.prs.length})`
                            : "List open PRs"}
                      </button>
                      {prsState?.error && (
                        <p
                          role="alert"
                          className="px-1 text-[10px] text-rose-400"
                        >
                          {prsState.error}
                        </p>
                      )}
                      {prsState?.prs && (
                        <ul className="max-h-40 overflow-y-auto rounded-md border border-neutral-800 bg-neutral-950 p-1 text-[10px]">
                          {prsState.prs.length === 0 ? (
                            <li className="px-1 py-0.5 text-neutral-600">
                              No open PRs
                            </li>
                          ) : (
                            prsState.prs.map((pr) => (
                  ...
                  {onGhCreateIssue && issueStep === "confirm" && (
                    <div className="rounded-md border border-amber-500/40 bg-amber-500/10 p-2 text-[11px] text-amber-200">
                      ...
                      <p className="mb-2 break-all text-amber-300/80">
                  ...
                  {onGhCreateIssue &&
                    ...
                    (issueDone?.error ? (
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="block rounded-md bg-neutral-800 py-1 text-center text-[11px] text-green-400 hover:bg-neutral-700"
                      >
            ...
            {settingsSets.length > 0 && (
              <div className="mt-2 border-t border-neutral-800 pt-2">
                ...
                {conformance === "loading" && (
                  <p className="px-1 text-[10px] text-neutral-600">checking…</p>
                )}
                {conformance?.error && (
                  <p role="alert" className="px-1 text-[10px] text-rose-400">
                    could not evaluate
                  </p>
                )}
                {conformance?.result &&
                  (() => {
                    const { presetName, checks, passCount, total } =
                      conformance.result;
                    const band =
                      passCount === total
                        ? "pass"

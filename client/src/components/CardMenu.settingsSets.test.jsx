...
const HYGIENE_PRESET = [
  { id: "hygiene", name: "Repo hygiene", description: "", checkCount: 5 },
];
...
describe("CardMenu — settings sets", () => {
  ...
  it("fetches and shows the conformance score on open", async () => {
    ...
    render(
      <CardMenu
        ...
        anchorRef={anchor}
        {...makeHandlers()}
        settingsSets={HYGIENE_PRESET}
        onGetConformance={onGetConformance}
      />,
    );
  ...
  it("re-fetches conformance when the repo changes", async () => {
    ...
    await waitFor(() =>
      expect(onGetConformance).toHaveBeenCalledWith(2, "hygiene"),
    );
  });
});

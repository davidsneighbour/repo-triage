...
export function Column({
  ...
  const ordered = useMemo(
    () => sortColumnRepos(repos, sortKey, sortDir),
    [repos, sortKey, sortDir],
  );
  const visible = useMemo(
    () => ordered.filter((r) => repoMatchesQuery(r, cq)),
    [ordered, cq],
  );
  ...
  return (
    <div
      {...devId("Column")}
      role="group"
      aria-label={`${col.title} column, ${repos.length} repositories`}
      className={cx(
    ...
    >
      <div
        className={cx(
          "mb-2 flex items-center justify-between rounded-lg border bg-neutral-900/40 px-3 py-2",
          acc.edge,
        )}
      >
        <div className="flex min-w-0 items-center gap-2">
          <span className={cx("h-2 w-2 shrink-0 rounded-full", acc.dot)} />
          <span className={cx("truncate text-sm font-semibold", acc.head)}>
            {col.title}
          </span>
      ...
      <div className="mb-2 flex items-center gap-1">
        ...
        <label className="relative block flex-1">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (colFilterCache)
                colFilterCache.current[col.key] = e.target.value;
            }}
          ...
          {cq && (
            <button
              ...
              onClick={() => {
                setCq("");
                if (colFilterCache) colFilterCache.current[col.key] = "";
              }}
        ...
        <label
          className="flex shrink-0 items-center"
          title={`Sort ${col.title}`}
        >
          <span className="sr-only">{`Sort ${col.title} column`}</span>
          <SortIcon
            className="mr-0.5 h-3 w-3 text-neutral-600"
            aria-hidden="true"
          />
          <select
            value={sort}

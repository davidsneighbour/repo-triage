...
describe('DevIdOverlay — toggle button', () => {
  it('renders inactive by default with aria-pressed=false', () => {
    render(<DevIdOverlay />);
    expect(screen.getByRole('button', { name: 'Toggle element identifier overlay' })).toHaveAttribute('aria-pressed', 'false');
  });

  it('activates on click and persists to localStorage', () => {
    render(<DevIdOverlay />);
    fireEvent.click(screen.getByRole('button', { name: 'Toggle element identifier overlay' }));
    expect(screen.getByRole('button', { name: 'Toggle element identifier overlay' })).toHaveAttribute('aria-pressed', 'true');
  ...
  it('deactivating clears any visible tooltip', async () => {
    ...
    render(<DevIdOverlay />);
    const toggle = screen.getByRole('button', { name: 'Toggle element identifier overlay' });

...
describe("EventLogView — load", () => {
  ...
  it("shows the event count across repos in the header", async () => {
    api.allActivity.mockResolvedValue({
      activity: [
        entry({ id: 1, repo_id: 1 }),
        entry({ id: 2, repo_id: 2, full_name: "me/beta" }),
      ],
    });
    ...
    expect(
      await screen.findByText("2 events across all repos"),
    ).toBeInTheDocument();
  });
  ...
  it("is not modal — role is region, not dialog", async () => {
    ...
    expect(
      await screen.findByRole("region", { name: "Event log" }),
    ).toBeInTheDocument();
  });
});

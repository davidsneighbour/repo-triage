...
function FieldsMenuPanel({ fields, onToggle, anchorRef, onClose }) {
  ...
  return createPortal(
    <>
      ...
      >
        <span className="block px-1 pb-1 text-[10px] uppercase tracking-widest text-neutral-500">
          Show fields
        </span>
...
export function FieldsMenu({ fields, onToggle }) {
  ...
  const hiddenCount = FIELD_OPTIONS.filter(
    (f) => fields[f.key] === false,
  ).length;
  return (
    <>
      <button
      ...
      >
        fields
        {hiddenCount
          ? ` (${FIELD_OPTIONS.length - hiddenCount}/${FIELD_OPTIONS.length})`
          : ""}
      </button>
      {open && (
        <FieldsMenuPanel

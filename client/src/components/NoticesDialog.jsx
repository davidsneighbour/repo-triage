...
export function NoticesDialog({
  ...
  const reload = useCallback(async () => {
    setLoading(true);
    try {
      const noticeRes = isAll
        ? await api.allNotices()
        : await api.repoNotices(scope);
            ...
            <p className="truncate text-[11px] text-neutral-500">
              ...
              {!isAll && (
                <button
                  ...
                  className="ml-2 text-neutral-400 underline hover:text-neutral-200"
                >
                  show all repos
                </button>
              )}
          ...
          {loading ? (
            ...
            activity.length === 0 ? (
              ...
              <ul className="space-y-1.5">
                {activity.map((e) => (
                  <li
                    key={e.id}
                    className="flex items-center justify-between gap-3 rounded-md bg-neutral-950 px-3 py-1.5"
                  >
                    <span className="text-xs text-neutral-300">
                      {activitySummary(e)}
                    </span>
                    <span
                      className="shrink-0 text-[10px] tabular-nums text-neutral-500"
            ...
            <ul className="space-y-2">
              {sorted.map((n) => (
                <li
                  key={n.id}
                  className="flex items-start justify-between gap-3 rounded-md bg-neutral-950 px-3 py-2"
                >
                  <div className="min-w-0">
                    {isAll && (
                      <p className="truncate text-[11px] font-medium text-neutral-300">
                        {n.full_name || `repo ${n.repo_id}`}
                      </p>
                    )}
                  ...
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    ...
                    {confirmId === n.id ? (
                      ...
                      <button
                        ...
                        className="text-neutral-600 hover:text-rose-300"
                      >

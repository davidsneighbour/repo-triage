...
export function SettingsDialog({
  ...
  const [defaultInactivityDays, setDefaultInactivityDays] = useState(
    String(
      settings?.defaultInactivityDays ?? defaults?.defaultInactivityDays ?? 7,
    ),
  );
  const [syncIntervalMinutes, setSyncIntervalMinutes] = useState(
    String(
      settings?.syncIntervalMinutes ?? defaults?.syncIntervalMinutes ?? 60,
    ),
  );
  ...
  const [scheduleCron, setScheduleCron] = useState(
    settings?.reportSchedule?.cron ?? "",
  );
  const [scheduleOutputPath, setScheduleOutputPath] = useState(
    settings?.reportSchedule?.outputPath ?? "",
  );
  ...
  const handleAddRule = async () => {
    ...
    if (!tag) {
      setRuleError("Tag is required.");
      return;
    }
    if (!Number.isFinite(days) || days < 1 || days > 365) {
      setRuleError("Days must be 1–365.");
      return;
    }
    setRuleError("");
  ...
  const handleSubmit = async (e) => {
    ...
    if (!Number.isFinite(days) || days < 1 || days > 365)
      errs.push("Review cycle must be between 1 and 365 days.");
    const mins = Number(syncIntervalMinutes);
    if (!Number.isFinite(mins) || mins < 1 || mins > 1440)
      errs.push("Sync interval must be between 1 and 1440 minutes.");
    if (errs.length) {
      setErrors(errs);
      return;
    }
    ...
    const reportSchedule =
      scheduleCron.trim() && scheduleOutputPath.trim()
        ? { cron: scheduleCron.trim(), outputPath: scheduleOutputPath.trim() }
        : null;
    try {
      await onSave({
        defaultInactivityDays: days,
  ...
  const fieldClass =
    "w-full rounded-md border border-neutral-700 bg-neutral-950 px-3 py-1.5 text-xs text-neutral-100 outline-hidden focus:border-neutral-500";
        ...
        <header className="flex items-center justify-between border-b border-neutral-800 px-4 py-3">
          <div>
            ...
            <p className="text-[11px] text-neutral-500">
              Overrides for .env defaults · persisted across restarts
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-[10px] text-neutral-600">
              Repos unseen for this many days return to Today. Env default:{" "}
              {defaults?.defaultInactivityDays ?? 7}d.
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-[10px] text-neutral-600">
              How often the server polls GitHub. Env default:{" "}
              {defaults?.syncIntervalMinutes ?? 60}m.
            </p>
          </div>
          ...
          <div>
            ...
            />
            <p className="mt-1 text-[10px] text-neutral-600">
              Changing owners triggers an immediate re-sync. Env default:{" "}
              {defaults?.githubOwners || "(token owner)"}.
            </p>
          </div>
          ...
          <div className="border-t border-neutral-800 pt-4">
            ...
            <p className="mb-2 text-[10px] text-neutral-600">
              Override review cadence per tag. Precedence: per-repo → tag rule →
              global default. Minimum days wins when multiple tags match.
            </p>
            {tagRules.length > 0 && (
            ...
            {ruleError && (
              <p role="alert" className="mb-1 text-[10px] text-rose-400">
                {ruleError}
              </p>
            )}
            <div className="flex gap-2">
              <input
                ...
                className={cx(
                  fieldClass,
                  "flex-1 placeholder:text-neutral-700",
                )}
              />
              <input
          ...
          <div className="border-t border-neutral-800 pt-4">
            ...
            <p className="mb-2 text-[10px] text-neutral-600">
              Writes all reports to disk after each sync when the cron schedule
              matches (5-field format, local time). Leave blank to disable.
            </p>
            <div className="space-y-2">
              <div>
                <label htmlFor="settings-sched-cron" className={labelClass}>
                  Cron expression
                </label>
                <input
              ...
              <div>
                <label htmlFor="settings-sched-path" className={labelClass}>
                  Output path
                </label>
                <input
              ...
              {lastExport && (
                <p className="text-[10px] text-neutral-500">
                  Last export: {lastExport.status === "ok" ? "✓" : "⚠"}{" "}

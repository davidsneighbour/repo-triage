...
describe('SettingsDialog tag rules', () => {
  it('renders existing tag rules when tagRules prop is provided', () => {
    const tagRules = [{ tag: 'infra', days: 14 }, { tag: 'docs', days: 30 }];
    render(<SettingsDialog settings={settings} defaults={defaults} tagRules={tagRules} onSave={vi.fn()} onClose={vi.fn()} />);
  ...
  it('calls onTagRuleDelete when the delete button for a rule is clicked', () => {
    ...
    render(<SettingsDialog settings={settings} defaults={defaults} tagRules={tagRules} onSave={vi.fn()} onTagRuleDelete={onTagRuleDelete} onClose={vi.fn()} />);
    fireEvent.click(screen.getByRole('button', { name: 'Delete tag rule for infra' }));
    expect(onTagRuleDelete).toHaveBeenCalledWith('infra');
  });

  it('shows ruleError when Add is clicked with an empty tag', () => {
    render(<SettingsDialog settings={settings} defaults={defaults} onSave={vi.fn()} onClose={vi.fn()} />);
    fireEvent.change(screen.getByLabelText('Tag rule days'), { target: { value: '14' } });
  ...
  it('shows ruleError when days is out of range', () => {
    render(<SettingsDialog settings={settings} defaults={defaults} onSave={vi.fn()} onClose={vi.fn()} />);
    fireEvent.change(screen.getByLabelText('Tag rule tag'), { target: { value: 'infra' } });
    fireEvent.change(screen.getByLabelText('Tag rule days'), { target: { value: '999' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add' }));
  ...
  it('calls onTagRuleSave with valid tag and days and clears inputs after', async () => {
    ...
    render(<SettingsDialog settings={settings} defaults={defaults} onSave={vi.fn()} onTagRuleSave={onTagRuleSave} onClose={vi.fn()} />);
    fireEvent.change(screen.getByLabelText('Tag rule tag'), { target: { value: 'infra' } });
    fireEvent.change(screen.getByLabelText('Tag rule days'), { target: { value: '14' } });
    fireEvent.click(screen.getByRole('button', { name: 'Add' }));
    await waitFor(() => expect(onTagRuleSave).toHaveBeenCalledWith('infra', 14));
...
describe('SettingsDialog report schedule', () => {
  ...
  it('pre-fills cron and outputPath from settings.reportSchedule', () => {
    const s = { ...settings, reportSchedule: { cron: '0 9 * * *', outputPath: '/tmp/out' } };
    render(<SettingsDialog settings={s} defaults={defaults} onSave={vi.fn()} onClose={vi.fn()} />);
  ...
  it('passes reportSchedule to onSave when cron and outputPath are filled', async () => {
    ...
    render(<SettingsDialog settings={settings} defaults={defaults} onSave={onSave} onClose={vi.fn()} />);
    fireEvent.change(screen.getByLabelText('Cron expression'), { target: { value: '0 8 * * 1-5' } });
    fireEvent.change(screen.getByLabelText('Output path'), { target: { value: '/data/reports' } });
    fireEvent.click(screen.getByRole('button', { name: 'Save settings' }));
  ...
  it('shows lastExport info when lastExport prop is provided with ok status', () => {
    const lastExport = { status: 'ok', timestamp: '2026-06-01T08:00:00.000Z', outputPath: '/data/reports' };
    render(<SettingsDialog settings={settings} defaults={defaults} lastExport={lastExport} onSave={vi.fn()} onClose={vi.fn()} />);
    expect(screen.getByText(/Last export/)).toBeInTheDocument();
    expect(screen.getByText(/Last export/).textContent).toMatch(/\/data\/reports/);
  });

  it('shows warning glyph for a non-ok lastExport status', () => {
    const lastExport = { status: 'error', timestamp: '2026-06-01T08:00:00.000Z' };
    render(<SettingsDialog settings={settings} defaults={defaults} lastExport={lastExport} onSave={vi.fn()} onClose={vi.fn()} />);

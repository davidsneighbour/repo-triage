  ...
  return (
    <div className="flex items-center gap-1 rounded-sm px-1 py-1">
      ...
      <button
        ...
        className="rounded-sm p-0.5 text-neutral-500 hover:text-emerald-300"
      >
      ...
      <button
        ...
        className="rounded-sm p-0.5 text-neutral-500 hover:text-neutral-200"
      >
...
function DeleteConfirmRow({ tag, count, onConfirm, onCancel }) {
  ...
  return (
    <div className="rounded-sm bg-neutral-800/60 px-1 py-1 text-[11px] text-neutral-300">
      <p className="px-0.5">
        Delete <span className="font-medium">#{tag}</span> from all {count} rep
        {count === 1 ? "o" : "os"}?
      </p>
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between px-1 pb-1">
          ...
          {selected.length > 0 && (
            <button
              onClick={() => onChange({ tags: [], mode: value.mode })}
        ...
        <div className="max-h-56 overflow-auto">
          {available.length === 0 ? (
            <p className="px-1 py-2 text-center text-[11px] text-neutral-600">
              no tags yet
            </p>
          ) : (
            available.map(({ tag, count }) => {
              ...
              return (
                <div
                  key={tag}
                  className="group flex items-center gap-2 rounded-sm px-1 py-1 text-[11px] text-neutral-300 hover:bg-neutral-800"
                >
        ...
        {onCreate && (
          <CreateTagRow
            onCreate={onCreate}
            existing={available.map((t) => t.tag)}
          />
        )}
      </div>
    </>,

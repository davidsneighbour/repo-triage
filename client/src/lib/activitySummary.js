...
export function activitySummary(entry) {
  ...
  if (entry.action === "inactivity")
    return `Review cadence → ${d.days == null ? "default" : `${d.days}d`}`;
  ...
  if (entry.action === "notice_add")
    return `Notice: ${String(d.body).slice(0, 60)}${d.body?.length > 60 ? "…" : ""}`;

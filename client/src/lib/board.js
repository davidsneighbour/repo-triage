...
export function filterRepos(
  ...
  return repos.filter((repo) => {
    ...
    if (tagFilter && !matchesTagFilter(repo, tagFilter.tags, tagFilter.mode))
      return false;
...
const LIST_SORTERS = {
  ...
  checked: (a, b) =>
    (a.checkedAgeDays ?? Infinity) - (b.checkedAgeDays ?? Infinity),
};
...
export function sortReposForList(repos, col = "repo", dir = "asc") {
  ...
  return [...repos].sort(
    (a, b) => cmp(a, b) * sign || (a.name || "").localeCompare(b.name || ""),
  );
}

...
export function sortIssues(issues, sort = "number", dir = "desc") {
  ...
  return [...issues].sort((a, b) => {
    ...
    if (sort === "repo")
      return (
        (a.repo_full_name || "").localeCompare(b.repo_full_name || "") * mul
      );
    if (sort === "updated") {
      const at = a.github_updated_at
        ? new Date(a.github_updated_at).getTime()
        : 0;
      const bt = b.github_updated_at

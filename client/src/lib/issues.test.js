...
describe("filterIssues", () => {
  ...
  it("filters by state", () => {
    ...
    expect(filterIssues(issues, { state: "closed" })).toEqual([
      issue({ number: 2, state: "closed" }),
    ]);
  });
...
describe("sortIssues", () => {
  ...
  it("sorts by number ascending", () => {
    ...
    expect(sortIssues(issues, "number", "asc").map((i) => i.number)).toEqual([
      1, 2, 3,
    ]);
  });
  ...
  it("sorts by title", () => {
    const issues = [
      issue({ number: 1, title: "zebra" }),
      issue({ number: 2, title: "apple" }),
    ];
    expect(sortIssues(issues, "title", "asc").map((i) => i.title)).toEqual([
      "apple",
      "zebra",
    ]);
  });
  ...
  it("sorts by repo_full_name", () => {
    ...
    expect(sortIssues(issues, "repo", "asc").map((i) => i.number)).toEqual([
      2, 1,
    ]);
  });
  ...
  it("sorts by updated timestamp", () => {
    ...
    expect(sortIssues(issues, "updated", "desc").map((i) => i.number)).toEqual([
      2, 1,
    ]);
  });
  ...
  it("treats a missing updated timestamp as oldest", () => {
    ...
    expect(sortIssues(issues, "updated", "asc").map((i) => i.number)).toEqual([
      1, 2,
    ]);
  });

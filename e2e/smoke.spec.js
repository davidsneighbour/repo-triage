...
test.describe("smoke", () => {
  test("board loads with correct columns and repo cards", async ({ page }) => {
    ...
    await expect(
      page.getByRole("group", { name: /Today column/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("group", { name: /Tomorrow column/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("group", { name: /Day after column/ }),
    ).toBeVisible();
  ...
  test("drag card onto another card moves it to the same column", async ({
    page,
  }) => {
    ...
    await expect(
      todayColumn.getByRole("group", { name: /gamma-repo/ }),
    ).toBeVisible();
  });

  test("search filter hides non-matching cards and clears", async ({
    page,
  }) => {
    await mockApi(page);
    await page.goto("/");
  ...
  test("adds a notice via card settings menu", async ({ page }) => {
    ...
    await menu
      .getByRole("textbox", { name: "New notice" })
      .fill("Test notice text");
  ...
  test("bulk ignore shows undo toast; undo restores the repo", async ({
    page,
  }) => {
    await mockApi(page);
    await page.goto("/");

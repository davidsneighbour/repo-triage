...
describe("parseOwners", () => {
  ...
  it("splits comma/space separated lists", () => {
    expect(parseOwners("davidsneighbour, dnbhq gohugo-ananke")).toEqual([
      "davidsneighbour",
      "dnbhq",
      "gohugo-ananke",
    ]);
  ...
  it("parses a JSON array", () => {
    expect(parseOwners('["davidsneighbour","dnbhq"]')).toEqual([
      "davidsneighbour",
      "dnbhq",
    ]);
  });
  ...
  it("de-duplicates case-insensitively, preserving first spelling/order", () => {
    expect(parseOwners("dnbhq, DNBHQ, gohugo-ananke")).toEqual([
      "dnbhq",
      "gohugo-ananke",
    ]);
  });
...
describe("token resolution (env vs gh CLI)", () => {
  it("uses GITHUB_TOKEN and never invokes gh when it is set", async () => {
    ...
    const fetchMock = vi
      .fn()
      .mockResolvedValue(makeRes({ body: [], headers: RATE_HEADERS }));
...
describe("fetchAllRepos — default (no owners configured)", () => {
  ...
  it("marks the token invalid and surfaces the GitHub message on 401", async () => {
    ...
    await expect(fetchAllRepos()).rejects.toThrow(
      /invalid or expired \(401\).*Bad credentials/s,
    );
    ...
    vi.stubGlobal(
      ...
      vi.fn().mockResolvedValue(
        makeRes({
          status: 403,
          headers: RATE_HEADERS,
          body: "abuse detection",
        }),
      ),
    );
    await expect(fetchAllRepos()).rejects.toThrow(
      /403 Forbidden.*abuse detection/s,
    );
  });
  ...
  it("maps fields, paginates, and clears authInvalid on success", async () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        makeRes({ body: lastPage, headers: RATE_HEADERS }),
      );
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("maps the cheap enrichment fields (forks/default branch/topics/license) with sane defaults", async () => {
    ...
    const fetchMock = routeFetch([
      [
        "/orgs/octocat/repos",
        () =>
          makeRes({ status: 404, body: "Not Found", headers: RATE_HEADERS }),
      ],
      [
        "/users/octocat/repos",
        () =>
      ...
      [
        "/user",
        () => makeRes({ body: { login: "me" }, headers: RATE_HEADERS }),
      ],
    ]);
    vi.stubGlobal("fetch", fetchMock);
...
describe("fetchAllRepos — configured owners", () => {
  it("loads the token owner via /user/repos (private included) when listed by login", async () => {
        ...
        () =>
          makeRes({
            body: [
              repo({
                id: 5,
                full_name: "davidsneighbour/x",
                owner: { login: "davidsneighbour", type: "User" },
                private: true,
              }),
            ],
            headers: RATE_HEADERS,
    ...
    expect(sourceStatus.owners[0]).toMatchObject({
      owner: "davidsneighbour",
      scope: "self",
    });
  });
  ...
  it("loads a member org via /orgs with no warning", async () => {
    ...
    const fetchMock = routeFetch([
        ...
        () =>
          makeRes({
            body: [
              repo({
                id: 9,
                full_name: "dnbhq/secret",
                owner: { login: "dnbhq", type: "Organization" },
                private: true,
              }),
            ],
            headers: RATE_HEADERS,
      ...
      [
        "/user/memberships/orgs/dnbhq",
        () => makeRes({ body: { state: "active" }, headers: RATE_HEADERS }),
      ],
      [
        "/user",
        () => makeRes({ body: { login: "me" }, headers: RATE_HEADERS }),
  ...
  it("falls back to public repos with a warning when not an org member", async () => {
    ...
    const fetchMock = routeFetch([
      [
        "/orgs/gohugo-ananke/repos",
        () =>
          makeRes({
            body: [
              repo({
                id: 7,
                full_name: "gohugo-ananke/theme",
                owner: { login: "gohugo-ananke", type: "Organization" },
              }),
            ],
            headers: RATE_HEADERS,
          }),
      ...
      [
        "/user/memberships/orgs/gohugo-ananke",
        () =>
          makeRes({ status: 404, body: "Not Found", headers: RATE_HEADERS }),
      ],
      [
    ...
    expect(sourceStatus.warnings.join(" ")).toMatch(
      /not a member of organization "gohugo-ananke"/,
    );
    expect(sourceStatus.owners[0]).toMatchObject({
      owner: "gohugo-ananke",
      scope: "public",
  ...
  it("warns and falls back to public on a 403 org listing", async () => {
    ...
    const fetchMock = routeFetch([
      [
        "/orgs/secret-org/repos",
        () =>
          makeRes({ status: 403, body: "forbidden", headers: RATE_HEADERS }),
      ],
      [
        "/users/secret-org/repos",
        () =>
          makeRes({
            body: [
              repo({
                id: 4,
                full_name: "secret-org/pub",
                owner: { login: "secret-org", type: "Organization" },
              }),
            ],
            headers: RATE_HEADERS,
          }),
    ...
    expect(sourceStatus.warnings.join(" ")).toMatch(
      /not authorized for organization "secret-org" \(403\)/,
    );
  });
  ...
  it("de-duplicates repos that appear under more than one owner", async () => {
    ...
    const shared = repo({
      id: 42,
      full_name: "a/shared",
      owner: { login: "a", type: "Organization" },
    });
    const fetchMock = routeFetch([
      [
        "/orgs/a/repos",
        () => makeRes({ body: [shared], headers: RATE_HEADERS }),
      ],
      [
        "/orgs/b/repos",
        () => makeRes({ body: [shared], headers: RATE_HEADERS }),
      ...
      [
        "/user/memberships/orgs/a",
        () => makeRes({ body: { state: "active" }, headers: RATE_HEADERS }),
      ],
      [
        "/user/memberships/orgs/b",
        () => makeRes({ body: { state: "active" }, headers: RATE_HEADERS }),
      ],
      [
        "/user",
  ...
  it("records a per-owner warning (not a hard failure) when one owner errors", async () => {
    ...
    const fetchMock = routeFetch([
        ...
        () =>
          makeRes({
            body: [
              repo({
                id: 1,
                full_name: "good-org/a",
                owner: { login: "good-org", type: "Organization" },
              }),
            ],
            headers: RATE_HEADERS,
          }),
      ...
      [
        "/user/memberships/orgs/good-org",
        () => makeRes({ body: { state: "active" }, headers: RATE_HEADERS }),
      ],
      [
        "/orgs/broken/repos",
        () => makeRes({ status: 500, body: "boom", headers: RATE_HEADERS }),
      ],
      [
        "/users/broken/repos",
        () => makeRes({ status: 500, body: "boom", headers: RATE_HEADERS }),
    ...
    const fetchMock = routeFetch([
      ...
      [
        ...
        () =>
          makeRes({
            body: [
              repo({ id: 8, full_name: "octocat/orphan", owner: undefined }),
            ],
            headers: RATE_HEADERS,
          }),
      ],
  ...
  it('counts a "pending" org membership as a member (private scope)', async () => {
    ...
    const fetchMock = routeFetch([
      ...
      [
        "/user/memberships/orgs/dnbhq",
        () => makeRes({ body: { state: "pending" }, headers: RATE_HEADERS }),
      ],
      [
        "/user",
        () => makeRes({ body: { login: "me" }, headers: RATE_HEADERS }),
  ...
  it("leaves viewerLogin unresolved when /user is unavailable (owner falls to org path)", async () => {
    ...
    expect(
      fetchMock.mock.calls.some(([url]) => url.includes("/user/repos")),
    ).toBe(false);
  });
  ...
  it("treats a non-active org membership as public-only", async () => {
    ...
    const fetchMock = routeFetch([
      ...
      [
        "/user/memberships/orgs/dnbhq",
        () => makeRes({ body: { state: "inactive" }, headers: RATE_HEADERS }),
      ],
      [
        "/user",
        () => makeRes({ body: { login: "me" }, headers: RATE_HEADERS }),
    ...
    expect(sourceStatus.warnings.join(" ")).toMatch(
      /not a member of organization "dnbhq"/,
    );
  });
  ...
  it("surfaces a 403 from a user listing as a per-owner warning", async () => {
    ...
    expect(repos).toEqual([]);
    expect(sourceStatus.warnings.join(" ")).toMatch(
      /Could not load "octocat".*403 Forbidden/,
    );
  });
  ...
  const graphqlResponse = (repos) => {
    ...
    for (const r of repos) {
      data[`r${r.id}`] = {
        ...
        releases: {
          nodes: [{ tagName: "v1.2.3", publishedAt: "2024-03-01T00:00:00Z" }],
        },
        defaultBranchRef: {
          target: {
...
describe("fetchRepoIssues", () => {
  ...
  it("stops paginating once a short page is returned", async () => {
    const fetchMock = routeFetch([
      [
        "/repos/o/r/issues",
        makeRes({ body: [issue()], headers: RATE_HEADERS }),
      ],
    ]);
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("throws on a non-ok response", async () => {
    ...
    await expect(fetchRepoIssues("o/r", "tok")).rejects.toThrow(
      /GitHub API 404/,
    );
  });
...
describe("fetchAllRepos — PAGINATE_VIA_GH mode", () => {
  ...
  it("calls gh api --paginate for the self repo path when no owners configured", async () => {
    const ghRepos = [
      repo({ id: 1, full_name: "me/r", owner: { login: "me", type: "User" } }),
    ];
    mockSpawnOnce({ stdout: JSON.stringify(ghRepos) + "\n" });
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue(makeRes({ body: {}, headers: RATE_HEADERS })),
    );

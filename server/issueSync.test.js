...
const rateLimit = { limit: 5000, remaining: 4999, used: 1, reset: null, lastChecked: null, authInvalid: false };
...
vi.mock('./github.js', () => ({
  ...
  buildResolveOwnerToken: vi.fn(() => () => ({ token: 'test-token', source: 'env' })),
...
const { fetchAllRepos, fetchRepoIssues, buildResolveOwnerToken } = await import('./github.js');
...
beforeEach(() => {
  ...
  buildResolveOwnerToken.mockReturnValue(() => ({ token: 'test-token', source: 'env' }));
...
describe('syncRepoIssues', () => {
  it('persists fetched issues and getStoredIssues returns them with parsed labels', async () => {
    fetchRepoIssues.mockResolvedValue([
      { number: 5, title: 'first', state: 'open', labels: ['bug'], body: 'x', html_url: 'https://x/5', github_updated_at: '2026-07-01T00:00:00Z' },
    ]);
    ...
    expect(stored[0]).toMatchObject({ number: 5, title: 'first', state: 'open', labels: ['bug'] });
  });

  it('upserts on re-sync instead of duplicating rows', async () => {
    fetchRepoIssues.mockResolvedValue([
      { number: 5, title: 'first (edited)', state: 'closed', labels: [], body: null, html_url: 'https://x/5', github_updated_at: '2026-07-02T00:00:00Z' },
    ]);

    await syncRepoIssues(REPO_A);
    const stored = getStoredIssues(REPO_A.id);
    expect(stored).toHaveLength(1);
    expect(stored[0]).toMatchObject({ title: 'first (edited)', state: 'closed' });
  });

  it('returns ok:false and records a warning when no token is available', async () => {
    const result = await syncRepoIssues(REPO_B, () => ({ token: null, source: null }));

...
describe('POST /api/repos/:id/issues/sync', () => {
  it('fetches and stores issues, then GET reflects them', async () => {
    fetchRepoIssues.mockResolvedValue([
      { number: 3, title: 'triage this', state: 'open', labels: ['help wanted'], body: 'text', html_url: 'https://x/3', github_updated_at: '2026-07-03T00:00:00Z' },
    ]);

    const syncRes = await request(app).post(`/api/repos/${REPO.id}/issues/sync`);
    ...
    expect(getRes.body.issues[0]).toMatchObject({ number: 3, title: 'triage this', labels: ['help wanted'] });
  });
...
describe('PUT /api/repos/:id/issue-sync', () => {
  it('disables and re-enables issue sync for a repo', async () => {
    let res = await request(app).put(`/api/repos/${REPO.id}/issue-sync`).send({ enabled: false });
    expect(res.status).toBe(200);
    ...
    res = await request(app).put(`/api/repos/${REPO.id}/issue-sync`).send({ enabled: true });
    expect(res.body).toEqual({ ok: true, syncEnabled: true });
  });

  it('returns 404 for an unknown repo id', async () => {
    const res = await request(app).put('/api/repos/999999/issue-sync').send({ enabled: false });
    expect(res.status).toBe(404);
  });
});

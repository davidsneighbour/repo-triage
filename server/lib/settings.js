...
export const DEFAULT_INACTIVITY_DAYS_ENV = Number(
  process.env.DEFAULT_INACTIVITY_DAYS || 7,
);
export const DAY_ROLLOVER_HOUR = Math.min(
  23,
  Math.max(0, Math.floor(Number(process.env.DAY_ROLLOVER_HOUR ?? 4)) || 0),
);
export const SYNC_ON_STARTUP = process.env.SYNC_ON_STARTUP !== "false";
export const SYNC_AUTO = process.env.SYNC_AUTO !== "false";
export const SYNC_INTERVAL_MINUTES_ENV = Math.max(
  1,
  Number(process.env.SYNC_INTERVAL_MINUTES || 60),
);
export const ENRICH_METADATA =
  (process.env.ENRICH_METADATA || "").toLowerCase() === "true";
...
export const ISSUE_SYNC_INTERVAL_MINUTES_ENV = Math.max(
  1,
  Number(process.env.ISSUE_SYNC_INTERVAL_MINUTES || 10080),
);
...
export function getEffectiveInactivityDays() {
  const v = Number(
    getSetting("default_inactivity_days") ?? DEFAULT_INACTIVITY_DAYS_ENV,
  );
...
export function getEffectiveSyncIntervalMinutes() {
  const v = Number(
    getSetting("sync_interval_minutes") ?? SYNC_INTERVAL_MINUTES_ENV,
  );

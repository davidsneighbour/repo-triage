...
describe("runMigrations", () => {
  ...
  it("back-fills tag_registry from tags already present in repo_tag", () => {
    ...
    const tags = db
      .prepare("SELECT tag FROM tag_registry")
      .all()
      .map((r) => r.tag);
    ...
    const migrations = [
      {
        ...
        description: "step 1",
        up(d) {
          d.exec("CREATE TABLE t1 (id INTEGER)");
        },
      },
      {
        version: v2,
  ...
  it("applies multiple pending migrations in version order (multi-step upgrade)", () => {
    const migrations = [
      ...
      },
      {
        ...
        description: "third",
        up(d) {
          d.exec("CREATE TABLE c (x INTEGER)");
        },
      },
    ];
    runMigrations(db, migrations);
  ...
  it("back-fills ignored/checked_at/snooze_until on a pre-column repo_state table", () => {
    ...
    const row = db
      .prepare("SELECT checked_at FROM repo_state WHERE repo_id = 1")
      .get();
  ...
  it("creates the repo_issue table and issue_sync_enabled column in the 2026070601 migration", () => {
    ...
    const indexes = db
      .prepare(
        `SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='repo_issue'`,
      )
      .all()
      .map((r) => r.name);
    expect(indexes).toContain("idx_repo_issue_repo_id");

    const repoStateCols = db
      .prepare("PRAGMA table_info(repo_state)")
      .all()
      .map((r) => r.name);
  ...
  it("back-fills the flagged column on a pre-existing repo_issue table without one", () => {
    ...
    const row = db
      .prepare(
        "SELECT flagged FROM repo_issue WHERE repo_id = 1 AND number = 1",
      )
      .get();

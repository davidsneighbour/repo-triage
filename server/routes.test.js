...
describe("checked_at reflects the real review time, not the scheduling anchor", () => {
  beforeAll(async () => {
    ...
    await request(app)
      .post(`/api/repos/${REPO.id}/inactivity`)
      .send({ days: null });
  });
...
describe("POST /api/repos/:id/priority", () => {
  it("rejects a priority outside 1|2|3|null with 400", async () => {
    const res = await request(app)
      .post(`/api/repos/${REPO.id}/priority`)
      .send({ priority: 5 });
    expect(res.status).toBe(400);
  });
...
describe("POST /api/repos/bulk", () => {
  ...
  it("bulk-ignores repos in a single transaction", async () => {
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "ignore", ids: [REPO.id] });
    expect(res.status).toBe(200);
  ...
  it("bulk-unignores repos", async () => {
    ...
    await request(app)
      .post("/api/repos/bulk")
      .send({ action: "unignore", ids: [REPO.id] });
    const board = await request(app).get("/api/repos");
    expect(board.body.repos.find((r) => r.id === REPO.id).ignored).toBe(false);
  });
  ...
  it("bulk-checks repos with daysAgo param", async () => {
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "check", ids: [REPO.id], daysAgo: 0 });
    expect(res.status).toBe(200);
    const board = await request(app).get("/api/repos");
    expect(board.body.repos.find((r) => r.id === REPO.id).checkedAgeDays).toBe(
      0,
    );
  });
  ...
  it("bulk-clears repos", async () => {
    ...
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "clear", ids: [REPO.id] });
    expect(res.status).toBe(200);
    const board = await request(app).get("/api/repos");
    expect(
      board.body.repos.find((r) => r.id === REPO.id).checkedAgeDays,
    ).toBeNull();
  });
  ...
  it("bulk-sets priority", async () => {
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "priority", ids: [REPO.id], priority: 2 });
    expect(res.status).toBe(200);
    const board = await request(app).get("/api/repos");
    ...
    await request(app)
      .post("/api/repos/bulk")
      .send({ action: "priority", ids: [REPO.id], priority: null });
  });

  it("bulk-tags and bulk-untags repos", async () => {
    await request(app)
      .post("/api/repos/bulk")
      .send({ action: "tag", ids: [REPO.id], tag: "bulk-test" });
    let board = await request(app).get("/api/repos");
    expect(board.body.repos.find((r) => r.id === REPO.id).tags).toContain(
      "bulk-test",
    );

    await request(app)
      .post("/api/repos/bulk")
      .send({ action: "untag", ids: [REPO.id], tag: "bulk-test" });
    board = await request(app).get("/api/repos");
    expect(board.body.repos.find((r) => r.id === REPO.id).tags).not.toContain(
      "bulk-test",
    );
  });
  ...
  it("returns 400 for invalid priority value", async () => {
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "priority", ids: [REPO.id], priority: 99 });
    expect(res.status).toBe(400);
  ...
  it("returns 400 when tag is empty for tag action", async () => {
    const res = await request(app)
      .post("/api/repos/bulk")
      .send({ action: "tag", ids: [REPO.id], tag: "" });
    expect(res.status).toBe(400);
...
describe("notices", () => {
  ...
  it("adds notices, surfaces latest + count on the board, and lists them newest-first", async () => {
    ...
    expect(list.body.notices.map((n) => n.body)).toEqual([
      "second note",
      "first note",
    ]);
  });
  ...
  it("returns all notices sortable by date and repo name", async () => {
    ...
    expect(
      byRepo.body.notices.every(
        (n) => typeof n.full_name === "string" || n.full_name === null,
      ),
    ).toBe(true);
  });
...
describe("tags", () => {
  ...
  it("adds normalised tags, dedupes, and exposes them on the board payload", async () => {
    ...
    await request(app)
      .post(`/api/repos/${REPO.id}/tags`)
      .send({ tag: "infra" }); // dup after normalise
  ...
  it("creates a registry-only tag with zero usage that shows up in the tag list", async () => {
    ...
    const all = await request(app).get("/api/tags");
    expect(all.body.tags.find((t) => t.tag === "unused")).toMatchObject({
      tag: "unused",
      count: 0,
    });
  });
  ...
  it("renames a tag across every repo that carries it", async () => {
    ...
    const rename = await request(app)
      .put("/api/tags/old-name")
      .send({ newTag: "new-name" });
  ...
  it("deletes a tag without resetting check state by default", async () => {
    ...
    const checkedBefore = before.body.repos.find(
      (r) => r.id === REPO.id,
    ).checked_at;
    ...
    const after = await request(app).get("/api/repos");
    expect(after.body.repos.find((r) => r.id === REPO.id).checked_at).toBe(
      checkedBefore,
    );
  });
  ...
  it("deletes a tag and resets check state for affected repos when resetCheck=true", async () => {
    ...
    const before = await request(app).get("/api/repos");
    expect(
      before.body.repos.find((r) => r.id === REPO.id).checked_at,
    ).not.toBeNull();
...
describe("POST /api/refresh", () => {
  it("queues a background sync and returns immediately without blocking on GitHub", async () => {
    let resolveFetch;
    fetchAllRepos.mockImplementationOnce(
      () =>
        new Promise((r) => {
          resolveFetch = r;
        }),
    ...
    expect(res.status).toBe(200);
    expect(res.body).toEqual(
      expect.objectContaining({ ok: true, queued: true, syncing: true }),
    );
  ...
  it("records a string rejection (no .message) via the e.message || e fallback", async () => {
    fetchAllRepos.mockRejectedValueOnce(
...
describe("backup & restore (runs last — mutates shared triage state)", () => {
  ...
  it("skips invalid tag_registry entries and defaults a missing created_at", async () => {
    ...
    const all = await request(app).get("/api/tags");
    expect(all.body.tags.find((t) => t.tag === "no-date")).toMatchObject({
      tag: "no-date",
      count: 0,
    });
  });
  ...
  it("round-trips a registry-only (unused) tag through backup/restore", async () => {
    ...
    const backup = await request(app).get("/api/backup");
    expect(
      backup.body.tag_registry.some((t) => t.tag === "unused-backup"),
    ).toBe(true);

    await request(app).post("/api/restore").send(backup.body);

    const all = await request(app).get("/api/tags");
    expect(all.body.tags.find((t) => t.tag === "unused-backup")).toMatchObject({
      tag: "unused-backup",
      count: 0,
    });
  });
...
describe("GET /api/settings + PUT /api/settings", () => {
  it("returns env defaults when no DB overrides exist", async () => {
    ...
    expect(res.body.defaults).toEqual(
      expect.objectContaining({ defaultInactivityDays: 7 }),
    );
  });
  ...
  it("PUT stores overrides and GET reflects them", async () => {
    const put = await request(app)
      .put("/api/settings")
      .send({ defaultInactivityDays: 14, syncIntervalMinutes: 30 });
  ...
  it("PUT rejects out-of-range defaultInactivityDays", async () => {
    const res = await request(app)
      .put("/api/settings")
      .send({ defaultInactivityDays: 0 });
    expect(res.status).toBe(400);
    expect(res.body.errors).toEqual(
      expect.arrayContaining([
        expect.stringContaining("defaultInactivityDays"),
      ]),
    );
  });
  ...
  it("PUT rejects out-of-range syncIntervalMinutes", async () => {
    const res = await request(app)
      .put("/api/settings")
      .send({ syncIntervalMinutes: 9999 });
    expect(res.status).toBe(400);
    expect(res.body.errors).toEqual(
      expect.arrayContaining([expect.stringContaining("syncIntervalMinutes")]),
    );
  });
  ...
  it("PUT accepts null to clear reportSchedule", async () => {
    await request(app)
      .put("/api/settings")
      .send({ reportSchedule: { cron: "0 9 * * *", outputPath: "/tmp" } });
  ...
  it("PUT rejects invalid cron in reportSchedule", async () => {
    const res = await request(app)
      .put("/api/settings")
      .send({ reportSchedule: { cron: "bad", outputPath: "/tmp" } });
    expect(res.status).toBe(400);
...
describe("GET /api/prefs + PUT /api/prefs", () => {
  ...
  it("PUT stores prefs and GET reads them back", async () => {
    const payload = {
      ...
      groupBy: "day",
      fields: { lang: false },
      filters: { showOwn: true, showForks: false, showArchived: false },
      showIgnored: false,
      tagFilter: { tags: ["bug"], mode: "any" },
      priorityFilter: [1, 2],
    };
  ...
  it("PUT strips unknown keys", async () => {
    await request(app)
      .put("/api/prefs")
      .send({ density: "comfortable", unknown_key: "should be dropped" });
    const get = await request(app).get("/api/prefs");
...
describe("gh quick-action endpoints", () => {
  ...
  it("POST /gh/issue creates an issue and returns url + number", async () => {
    ...
    const res = await request(app)
      .post(`/api/repos/${REPO.id}/gh/issue`)
      .send({ title: "Found a bug", body: "Steps to reproduce..." });
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
...
describe("GET /api/repos/:id/activity", () => {
  ...
  it("logs a tag_add action", async () => {
    await request(app)
      ...
      .send({ tag: "log-test" });
    const res = await request(app).get(`/api/repos/${REPO.id}/activity`);
    const entry = res.body.activity.find(
      (e) => e.action === "tag_add" && e.detail?.tag === "log-test",
    );
    expect(entry).toBeDefined();
  });
...
describe("GET /api/activity", () => {
  ...
  it("includes repo_id and full_name on each entry, across repos", async () => {
    ...
    const res = await request(app).get("/api/activity");
    const entry = res.body.activity.find(
      (e) => e.repo_id === REPO.id && e.action === "check",
    );
    expect(entry).toBeDefined();
...
describe("GET/POST /api/undo + POST/DELETE /api/undo/:id", () => {
  ...
  it("POST /undo/:id executes a restoreState op", async () => {
    ...
    const post = await request(app)
      .post("/api/undo")
      .send({ label: "Restore state", ops: restoreOps });
    expect(post.status).toBe(200);
    const id = post.body.id;
  ...
  it("POST /undo/:id with ignored:true applies ignore to the repo", async () => {
    ...
    const post = await request(app)
      .post("/api/undo")
      .send({ label: "Undo unignore", ops: ignoreOps });
    const id = post.body.id;

...
router.get("/tokens", (_req, res) => {
  const tokens = db
    .prepare("SELECT id, name, owners, created_at FROM tokens ORDER BY id")
    .all();
  res.json({ tokens });
});
...
router.post("/tokens", (req, res) => {
  ...
  const ownersStr = Array.isArray(owners)
    ...
    : typeof owners === "string"
      ...
      : "";
  try {
    const { encrypted, iv, authTag, salt } = encryptToken(
      token.trim(),
      passphrase,
    );
    const result = db
...
router.put("/tokens/:id", (req, res) => {
  ...
  const setCols = Object.keys(updates)
    .map((k) => `${k} = ?`)
    .join(", ");
  db.prepare(`UPDATE tokens SET ${setCols} WHERE id = ?`).run(

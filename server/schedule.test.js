...
describe("effectiveState", () => {
  ...
  it("returns unchecked for never checked repos", () => {
    const state = effectiveState(
      { priority_set_at: null, inactivity_days: null },
      7,
      nowMs,
    );
  ...
  it("returns future column for recent checks", () => {
    ...
    const state = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 7 },
      7,
      nowMs,
    );
  ...
  it("clamps board offset to visible column count", () => {
    ...
    const state = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 14 },
      7,
      nowMs,
    );
  ...
  it("derives checkedAgeDays from checked_at, independent of the back-dated anchor", () => {
    ...
    const state = effectiveState(
      { priority_set_at: anchor, checked_at: checked, inactivity_days: 7 },
      7,
      nowMs,
    );
  ...
  it("reports checkedAgeDays null when scheduled but never actually checked", () => {
    ...
    const state = effectiveState(
      { priority_set_at: anchor, checked_at: null, inactivity_days: 7 },
      7,
      nowMs,
    );
    expect(state.column).toBe("day-0");
  ...
  it("supports inactivity values 0, 1, 7, 14 and null", () => {
    ...
    const zeroDays = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 0 },
      7,
      nowMs,
    );
    ...
    const oneDay = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 1 },
      7,
      nowMs,
    );
    ...
    const sevenDays = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 7 },
      7,
      nowMs,
    );
    ...
    const fourteenDays = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: 14 },
      7,
      nowMs,
    );
    ...
    const defaultFromNull = effectiveState(
      { priority_set_at: checkedAt, inactivity_days: null },
      7,
      nowMs,
    );
  ...
  describe("day rollover hour", () => {
    ...
    it("keeps the same board day for two instants in the same rollover window", () => {
      ...
      expect(boardDayIndex(beforeRollover, 4)).toBe(
        boardDayIndex(lateYesterday, 4),
      );
      // After 04:00 it has advanced to the next board day.
      expect(boardDayIndex(afterRollover, 4)).toBe(
        boardDayIndex(beforeRollover, 4) + 1,
      );
    });
    ...
    it("only advances a card toward Today once the rollover hour passes", () => {
      ...
      const checkedAt = new Date(
        new Date(2026, 0, 9, 2, 0, 0).getTime(),
      ).toISOString();

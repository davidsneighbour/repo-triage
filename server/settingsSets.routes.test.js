...
const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'repo-dash-settingssets-'));
...
const HYGIENIC_REPO = {
  id: 301, full_name: 'me/hygienic', name: 'hygienic',
  description: 'a well-kept repo', license: 'MIT', topics: ['cli'], has_issues: true, has_wiki: false,
};
const BARE_REPO = {
  id: 302, full_name: 'me/bare', name: 'bare',
  description: null, license: null, topics: [], has_issues: false, has_wiki: true,
};
...
describe('GET /api/repos/:id/settings-sets/:presetId', () => {
  it('scores a hygienic repo at 5/5', async () => {
    const res = await request(app).get(`/api/repos/${HYGIENIC_REPO.id}/settings-sets/hygiene`);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ presetId: 'hygiene', presetName: 'Repo hygiene', passCount: 5, total: 5 });
  });

  it('scores a bare repo at 0/5', async () => {
    const res = await request(app).get(`/api/repos/${BARE_REPO.id}/settings-sets/hygiene`);
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ passCount: 0, total: 5 });
  });

  it('returns 404 for an unknown repo id', async () => {
    const res = await request(app).get('/api/repos/999999/settings-sets/hygiene');
    expect(res.status).toBe(404);
    expect(res.body.error).toMatch(/repo not found/);
  });

  it('returns 404 for an unknown preset id', async () => {
    const res = await request(app).get(`/api/repos/${HYGIENIC_REPO.id}/settings-sets/does-not-exist`);
    expect(res.status).toBe(404);
